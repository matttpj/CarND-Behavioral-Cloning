...
def loadImageAndMeasurement(dataPath, imagePath, measurement, images, measurements):
    ...
    Executes the following steps:
      - Loads the image from `dataPath` and `imagePath`.
...
def trainAndSave(model, inputs, outputs, modelFile, epochs=5):
    """
    Train the model `model` using 'mse' loss and 'adam' optimizer for the 'epochs'.
...
trainAndSave(model, X_train, y_train, 'models/model_nVidia.h5')
print('The End')
